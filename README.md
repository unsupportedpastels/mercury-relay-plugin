...
An admitted encrypted controller sees both
`capabilities.push_notifications_v1: true` and
`capabilities.push_notifications_v2: true` in `relay.lease.attached` and
`relay.status` only when the bridge is configured. The v1 registration contract
is unchanged: `relay.push.register` accepts exactly `{device_token, environment}`
(lowercase even-length hex, 32–200 characters; `environment: "sandbox"`) and
returns `{registered: true, wake_handle}`. `relay.push.unregister` accepts `{}`
and returns `{registered: false}`. These methods are handled inside the plugin,
...
`{resolved: true, durable_session_id, profile}`. Every other valid lookup returns
`{resolved: false}`: wrong device, authorization epoch or handle; expired or
already-consumed route; revoked/unregistered handle; or an event whose runtime
session has no binding proven by the recovery projection. Pending routes are
...
```bash
scripts/compat_gate.sh tests/test_push.py tests/test_push_lifecycle.py tests/test_push_binding.py tests/test_push_resolution.py
